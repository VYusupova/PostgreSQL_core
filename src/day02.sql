-- ex00 -- необходимо вернуть список названий пиццерий с рейтингом которые никто не посещал
SELECT NAME, 
          rating 
     FROM pizzeria 
...
-- ex01 -- вернуть потерянные даты с 1 по 10 января 2022 (включительно) 
...
-- ex02 -- вернуть весь список имен посетителей( или не посетителей)пиццерий 
...
--------------Обратите внимание на замену ‘-’ for NULL значений в столбцах.
    ...
    ORDER BY 1,2,3
-- ex03 -- вернемся к упраженению 1 перепешите ваш SQL с использованием СТЕ паттерна. перемести в часть СТЕ ваш "генератор дней". Вывод должен быть таким же как в упражнениии 1
...
-- ex04 --__________________________
...
---- ex05 -- __________________________
...
SELECT name
 ...
 WHERE age > 25
   AND gender = 'female'
ORDER BY name
--- ex06 -- __________________________
...
---- ex07 -- __________________________
...
---- ex08 -- __________________________
WITH pop (id) AS
(
...
---- ex09 -- __________________________
WITH pop (id) AS
(
 SELECT person_id
  FROM person_order
...
---- ex10 -- __________________________
