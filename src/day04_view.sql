/*Если нужно не просто проверить, но и заменить представление, если оно есть, используйте CREATE OR REPLACE VIEW. 
  если ваша задача — именно «создать, если нет», берите IF NOT EXISTS.
*/

-- ex00 --Создайте 2 представления базы данных (с похожими атрибутами, как у исходной таблицы)
----------на основе простой фильтрации пола лиц. 
...
-- ex03 -- Напишите SQL-скрипт который вернет пропущенные дни посещений в январе 2022. 
--Используейте представление `v_generated_dates` для этой задачи и отсортируйте результат по пропущенной дате. Пример вывода представлен ниже 
...
;
--ex 05 оздайте представление  `v_price_with_discount` 
-- -- которое возвращает заказы с именем, названием пиццы, ценой и вычисляемый столбик  `discount_price`(с применением 10% скидки и высчитвываемом по формуле   `price - price*0.1`).
-- -- Результат отсортируйте по имени и названию пиццы и округлите `discount_price` до целого типа 
...
SELECT DISTINCT pizzeria.name 
 ...
 WHERE person.name = 'Dmitriy'
   ...
   AND price < 800
;
--ex 07 Давайте обновим данные в нашем материализованном представлении `mv_dmitriy_visits_and_eats` из ex06. 
...
;
--ex 08 УДАЛИТЬ созданные представления 
